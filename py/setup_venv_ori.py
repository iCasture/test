...
# the caller's script name using a logging.Filter.
logger = get_logger(
    "setup_venv",
...
class _CallerScriptFilter(logging.Filter):
    """
    Logging filter that injects the name of the script that imported this module.

    This is useful when you want to trace which script triggered the logging output.

    The filter sets the `record.caller` attribute, which can then be used in the formatter string
    via `%(caller)s`.

    Attributes
    ----------
    record.caller : str
        The base filename (e.g. `main.py`) of the script that imported this module.
        If the caller cannot be determined, `unknown` is used.
    """

    def __init__(self) -> None:
        super().__init__()

        # List of files/patterns to exclude when looking for the caller
    ...
    def _find_frame_in_stack(self) -> str | None:
        """
        Find the first frame in the stack that doesn't match the exclude patterns.

        Returns
        -------
        str or None
            The name of the caller file, or `None` if no suitable frame is found.
        """
        # Get the current stack frames, skipping this function and filter()
        # stack = inspect.stack()[2:]

        # Get the current stack frames
        stack = inspect.stack()

        # Track files we've seen to avoid duplicates and infinite recursion
        ...
        for frame in stack:
            ...
            if any(pattern in caller_path for pattern in self.exclude_patterns):
                continue

            # Skip common internal Python modules
            if (
                "<" in caller_path  # Handles <frozen module> and similar
                or "site-packages" in caller_path  # Installed packages
                # or "python" in caller_path  # Python standard library
            ):
                continue

            # Skip venv-related files
            if (
                "venv" in caller_path.lower()
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        try:
            # First try to find the real caller by inspecting the stack
            frame_found = self._find_frame_in_stack()

            if frame_found:
                record.caller = frame_found
                return True

            # If that fails, try to determine the calling script from sys.argv
            if sys.argv and len(sys.argv) > 0:
                record.caller = Path(sys.argv[0]).name
                return True
...
logger.addFilter(_CallerScriptFilter())
